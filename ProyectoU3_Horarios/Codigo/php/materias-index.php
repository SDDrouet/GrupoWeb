            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        <h2 class="float-left">Materias Detalles</h2>
                        <a href="materias-create.php" class="btn btn-success float-right">Nuevo registro</a>
                        <a href="materias-index.php" class="btn btn-info float-right mr-2">Refrescar</a>
                        <a href="index.php" class="btn btn-secondary float-right mr-2">Atrás</a>
                    </div>

                    <div class="form-row">
                        <form action="materias-index.php" method="get">
                        <div class="col">
                    ...
                    $total_pages_sql = "SELECT COUNT(*) FROM materias";
                    ...
                    $sql = "SELECT * FROM materias ORDER BY $order $sort LIMIT $offset, $no_of_records_per_page";
                    $count_pages = "SELECT * FROM materias";


                    if(!empty($_GET['search'])) {
                        $search = ($_GET['search']);
                        $sql = "SELECT * FROM materias
                            WHERE CONCAT_WS (cod_materia,nombre_materia,departamento,horas_semana)
                            LIKE '%$search%'
                            ORDER BY $order $sort
                            ...
                            echo "<table class='table table-bordered table-striped'>";
                                echo "<thead>";
                                    echo "<tr>";
										...
										echo "<th><a href=?search=$search&sort=&order=horas_semana&sort=$sort>Horas semanales</th>";
                                ...
                                while($row = mysqli_fetch_array($result)){
                                    echo "<tr>";
                                    echo "<td>" . htmlspecialchars($row['cod_materia']) . "</td>";echo "<td>" . htmlspecialchars($row['nombre_materia']) . "</td>";echo "<td>" . htmlspecialchars($row['departamento']) . "</td>";echo "<td>" . htmlspecialchars($row['horas_semana']) . "</td>";
                                        echo "<td>";
